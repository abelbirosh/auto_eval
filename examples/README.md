...
| [`dataset-hard-lookup.jsonl`](dataset-hard-lookup.jsonl) | Twenty questions built to be hard for a single query: every answer sits inside a document rather than a headline, with a near-identical competing number or name nearby. All twenty were published between February 2025 and July 2026, and every answer was checked against the source in the row. |
